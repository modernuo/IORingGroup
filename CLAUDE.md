...
- **Send buffer growth**: while a socket has a `RetiringSendBuffer`, nothing is posted from its current `SendBuffer`; a retiring buffer is released only by the completion that drains it, so never attach an empty buffer as retiring.
