...
- `RingSocket` — Managed socket wrapping an OS handle with pre-registered send/recv buffers. Tracks in-flight operation flags and generation counter for stale completion detection. `RecvBuffer` is replaced by the manager on recv promotion; read it per event.
